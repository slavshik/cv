...
- **The Projects section is gone, and the titles with it.** It was four names
  and their dates — Chain Cube 3D, Hidden Object: Brain Teaser, Color Galaxy,
  Fireballs and Friends — with no descriptions, because `/details/projects/` on
  LinkedIn carries names, dates and the associated company and nothing else. A
  bare title says nothing a reader can use, and the work entries already
  describe the same games without naming them. The better home for the names is
  inside those highlights, but which title is which game is Alexander's to say,
  not a guess to be made from dates. Ask him, then name them there rather than
  restoring the list.
